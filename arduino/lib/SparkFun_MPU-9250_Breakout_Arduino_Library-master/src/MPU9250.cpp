...
void MPU9250::getMres()
{
  if(Mscale == MFS_14BITS)
      mRes = 49120.0f / 8190.0f; // Proper scale to return milliGauss
  else
      mRes = 49120.0f / 32760.0f; // Proper scale to return milliGauss
}
...
void MPU9250::MPU9250SelfTest(float * destination)
{
  ...
  getAverageValuesGyroAndAcclerometer(aAvg,gAvg);
  ...
  getAverageValuesGyroAndAcclerometer(aSTAvg,gSTAvg);
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    // FT[Xa] FT[Ya] FT[Za] FT[Xg] FT[Yg] FT[Zg] factory trim calculation
    factoryTrim[i] = (float)(2620/1<<FS)*(pow(1.01 ,((float)selfTest[i] - 1.0) ));
  }
...
void MPU9250::getAverageValuesGyroAndAcclerometer(int32_t *aAvg /*[3]*/,int32_t *gAvg /*[3]*/)
{
...
void MPU9250::magCalMPU9250(float * bias_dest, float * scale_dest)
{
  ...
  int16_t mag_max[3]  = {INT16_MIN, INT16_MIN, INT16_MIN},
          mag_min[3]  = {INT16_MAX, INT16_MAX, INT16_MAX},
          mag_temp[3] = {0, 0, 0};
  int delayMs;
  ...
  for (int jj = 0; jj < 3; jj++)
  {
    ...
    // Get 'average' x(0) y(1) z(2) mag bias in counts
    mag_bias[jj]  = (mag_max[jj] + mag_min[jj]) / 2;
    // Save mag biases in G for main program
    bias_dest[jj] = (float)mag_bias[jj] * mRes * factoryMagCalibration[jj];
    // Get soft iron correction estimate
    // Get average x(0) y(1) z(2) axis max chord length in counts
    mag_scale[jj]  = (mag_max[jj] - mag_min[jj]) / 2;
  }
...
uint8_t MPU9250::readByte(uint8_t address, uint8_t subAddress)
{
  ...
  Wire.requestFrom(address, (uint8_t) 1); // Read one byte from slave
                                          // register address
  // Fill Rx buffer with result; Return data read from slave register
  return Wire.read();
}
